...
class APIKeyManager:
    ...
    def get_user_credentials_by_principal_id(self, db: Session, 
                                           ...
                                           is_testnet: bool = True) -> Optional[Dict[str, str]]:
        """
        Get user exchange credentials by ICP Principal ID
        ...
        """
        try:
            # Find user by principal ID through subscription
            subscription = db.query(models.Subscription).filter(
                models.Subscription.user_principal_id == user_principal_id
            ).first()
            
            if not subscription:
                logger.warning(f"No subscription found for principal ID: {user_principal_id}")
                return None
            
            # Get credentials for the user
            return self.get_user_exchange_credentials(
                db, subscription.user_id, exchange, is_testnet
            )
...
def get_bot_api_keys(user_principal_id: str, exchange: str = "BINANCE", 
    ...
    def store_user_exchange_credentials_by_principal_id(
        ...
        """
        Store exchange credentials using principal ID mapping
        ...
        """
        try:
            # Find user via user_principals mapping
            user_principal = db.query(models.UserPrincipal).filter(
                models.UserPrincipal.principal_id == principal_id,
                models.UserPrincipal.status == models.UserPrincipalStatus.ACTIVE
            ).first()
            
            if not user_principal:
                logger.error(f"No active principal mapping found for principal ID: {principal_id}")
                return False
            
            # Use existing method with user_id
            return self.store_user_exchange_credentials(
                db=db,
